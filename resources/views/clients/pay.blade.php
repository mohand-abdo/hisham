...
@section('content')
        ...
        <div class="col-12">          
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary" style="float:right">تفاصيل  الدين</h6>
                ...
                <div class="card-body">
                    <div class="row">    
                        <div class="col-md-6 col-xs-12">
                            ...
                            <input type="text" class="form-control dis" disabled value="{{$cli_account->client->name}}">
                        </div>
                        <div class="col-md-6 col-xs-12">
                            <label for="">التاريخ </label>
                            <input type="text" class="form-control dis" disabled value="{{date_format($cli_account->created_at,'Y-m-d')}}">
                        </div>
                    </div>
                    <div class="row">    
                        <div class="col-md-4 col-xs-12">
                            <label for="">المبلغ االكلي</label>
                            <input type="text" class="form-control dis" disabled value="{{number_format($cli_account->total_price)}}">
                        </div>
                        <div class="col-md-4 col-xs-12">
                            <label for="">المتبقي</label>
                            <input type="text" class="form-control dis" disabled value="{{number_format($cli_account->total_price - $cli_account->pay_price) }}">
                        </div>
                        <div class="col-md-4 col-xs-12">
                            <label for="">المدفوع</label>
                            <input type="text" class="form-control dis" disabled value="{{number_format($cli_account->pay_price)}}">
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal fade pay" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <form action="{{route('client.account_pay')}}" method="post" class="modal_form">
            @csrf
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <input type="hidden" name="cli_account_id" value="{{$cli_account->id}}">
    ...
    <script>
        $(document).ready(function(){
            ...
            $('body').on('hide.bs.modal',function(){
                ...
                $('.btn-add').addClass('disabled');
                $('.modal_form').attr('action','{{ route("client.account_pay")}}');
