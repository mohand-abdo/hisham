...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-12">
            <form action="{{route('purchase.store')}}" method="post" class="puchase_form">
                @csrf
                <div class="card shadow mb-4 col-12">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-primary" style="float:right">فاتورة مشتربات</h6>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-4">
                                <div class="form-group">
                                    <label for="supplier_id" class="supplier_id"> المورد</label>
                            ...
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <input id="inv_no" class="form-control"  name="inv_no" value="{{ old('inv_no') }}"/>
                                </div>
                            </div>
                            ...
                            <div class="col-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="inv_date" class="form-control"  name="inv_date" value="{{ old('inv_date') }}" autocomplete="off"/>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-4 safe">
                                <div class="form-group">
                                    <span class="require-insert">*</span>
                                    <label for="safe" > الخزنة</label>
                                    <select name="safe" id="safe" class="form-control">
                                        @foreach ($safes as $index => $safe)
                                            <option value="{{ $safe->id }}" {{ old('safe') == $safe->id ? 'selected':'' }}>{{ $safe->name}}</option>                                            
                                        @endforeach
                                    </select>
                                </div>
                            </div>

                            {{-- <div class="pay_type col-8"> --}}
                                    ...
                                    {{-- <div class="col-6 uncash_type d-none">
                                        <div class="form-group">
                                            ...
                                            <select name="uncash_type" id="uncash_type" class="form-control">
                                                ...
                                                <option value="1" {{ old('uncash_type') == 1 ? 'selected':'' }}>غير مشروط</option>
                                                <option value="2" {{ old('uncash_type') == 2 ? 'selected':'' }}>مشروط</option>
                                            </select>
                                        </div>
                                    </div> --}}
                                    
                            <div class="col-4 pay_date d-none">
                                <div class="form-group">
                                    <span class="require-insert">*</span>
                                    ...
                                    <table class="table table-right align-items-center table-flush">
                                        ...
                                        <tbody class="product-list">
                                            @foreach ($items as $item)
                                                <tr>
                                                    <td>{{$item->name}}</td>
                                                    <td class="stock-{{$item->id}} qty">{{$item->stocks()->where('stock_id', 1)->exists() ? $item->stocks()->find(1)->pivot->qty : '0'}}</td>
                                                    <td>{{number_format($item->purches_price)}}</td>
                                                    <td>
                                                        <a href="" class="btn btn-success btn-sm btn-add-product rounded-circle" id="product-{{$item->id}}" data-id="{{$item->id}}" data-name="{{$item->name}}" data-price="{{$item->purches_price}}" data-qty="{{$item->stocks()->where('stock_id', 1)->exists() ? $item->stocks()->find(1)->pivot->qty : '0'}}">
