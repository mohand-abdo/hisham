...
@section('title','تفاصيــل فاتورة المشتريات')
...
@section('content')
        ...
        <div class="col-12">          
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-primary" style="float:right">تفاصيل فاتورة مشتريات</h6>
                ...
                <div class="card-body">
                    <div class="row">    
                        <div class="col-md-3 col-xs-12">
                            ...
                            <input type="text" class="form-control dis" disabled value="{{'His-'. $purchase->id .'-RT'}}">
                        </div>
                        <div class="col-md-3 col-xs-12">
                            <label for="">العميل</label>
                            <input type="text" class="form-control dis" disabled value="{{$purchase->supplier->name}}">
                        </div>
                        <div class="col-md-3 col-xs-12">
                            <label for="">تاريخ الفاتورة</label>
                            <input type="text" class="form-control dis" disabled value="{{date_format($purchase->created_at,'Y-m-d')}}">
                        </div>
                        <div class="col-md-3 col-xs-12">
                            <label for="">تاريخ الاستحقاق</label>
                            <input type="text" class="form-control dis" disabled value="{{$purchase->pay_date}}">
                        </div>
                    </div>
                    <div class="row">    
                        <div class="col-md-3 col-xs-12">
                            <label for="">نوع الفاتورة</label>
                            <input type="text" class="form-control dis" disabled value="{{$purchase->uncash_type == 1 ? ' مشروط' : 'غير مشرروط'}}">
                        </div>
                        <div class="col-md-3 col-xs-12">
                            <label for="">اجمالي الفاتورة</label>
                            <input type="text" class="form-control dis" disabled value="{{number_format($purchase->total_price)}}">
                        </div>
                        <div class="col-md-3 col-xs-12">
                            <label for="">المتبقي</label>
                            <input type="text" class="form-control dis" disabled value="{{number_format($purchase->total_price - $purchase->sup_account->pay_price )}}">
                        </div>
                        <div class="col-md-3 col-xs-12">
                            <label for="">المدفوع</label>
                            <input type="text" class="form-control dis" disabled value="{{number_format($purchase->sup_account->pay_price)}}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="col-12 mb-4">
                    <div class="card shadow pb-3">
                        <div class="card-header py-3">
                            ...
                            <a href="{{-- {{ route('purchase.account.pay')}} --}}#" class="btn btn-primary" style="float:left" data-toggle="modal" data-target=".pay">سداد</a>
                            <div class="clearfix"></div>
                        </div>
                        <div class="card-body">
    ...
    <div class="modal fade pay" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true">
        <form action="{{route('purchase.account.pay')}}" method="post" class="modal_form">
            @csrf
    ...
    <script>
        $(document).ready(function(){
            ...
            $('body').on('hide.bs.modal',function(){
                ...
                $('.btn-add').addClass('disabled');
                $('.modal_form').attr('action','{{ route("purchase.account.pay")}}');
