                ...
                <div class="row">
                    <div class="col-6 pr-4">
                        <p>
                            ...
                            <span>{{date_format($purchase->created_at,'Y-m-d')}}</span>
                        </p>
                        <p>
                            <span class="ml-3">اسم المورد :</span>
                            <span>{{$purchase->supplier()->exists() ? $purchase->supplier->name : 'غير معروف'}}</span>
                        </p>
                    </div>
                    ...
                    <div class="col-6 pl-4">
                        <p>
                            ...
                            <span>{{'His-'.$purchase->id.'-RT'}}</span>
                        </p>
                        <p>
                            <span class="ml-3"> اسم المستخدم :</span>
                            <span>{{$purchase->user->f_name.' '.$purchase->user->l_name}}</span>
                        </p>
                    </div>
                </div>
                        ...
                        <tbody>
                            @foreach ($purchase->items as $index => $item)
                                <tr>
                                    ...
                                    <td class="text-center">{{ $item->purches_price }}</td>
                                    <td class="text-center">{{ number_format($item->purches_price * $item->pivot->qty) }}</td>
                                </tr>
                            @endforeach
                            <tr class="text-left">
                                <th colspan="4" class="text-center">المجــمـــوع</th>
                                <td class="text-center">{{ number_format($purchase->total_price) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
