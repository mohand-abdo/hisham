...
@section('title','كشف حساب بالديون لك')
    ...
    <div class="col-12">
        <div class="card shadow mb-4 ">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary" style="float:right">كشف حساب بالديون لك</h6>
            </div>
            <div class="card-body">
                            ...
                            <tbody>
                                @foreach($clients as $index => $client)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#" class="btn btn-primary btn-sm rounded-circle user_info" title="تفاصيل المورد" data-id="{{$client->client_id}}" data-url="{{route('client.show',$client->client_id)}}" data-toggle="modal" data-target=".user"><i class="fa fa-user"></i></a>
                                            <a href="{{ route('report.client.money',$client->client_id) }}" class="btn btn-info btn-sm rounded-circle" title="تفاصيل المديونات"><i class="fa fa-link"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
        ...
        $('body').on('click', '.user_info', function () { 
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('#name').val(data.name);
                    $('.modal-title').text('بيانات الزبون '+ data.name);
