    ...
    <div class="col-12">
        <div class="card shadow mb-4 ">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary" style="float:right">حساب البنك</h6>
            </div>
            <div class="card-body">
                @include('includes.messages')
                <div class="col-12">
                    <form class="d-flex justify-content-between mb-3" method="post" action="{{route('report.money.bank.search')}}"  >
                        @csrf
                        <div class="col-4">
                        ...
                        <select class="form-control ml-5" name="bank_type">
                            <option value="" >---اختر---</option>
                            @foreach ($banks as $bank)
                                <option value="{{$bank->id}}" {{ $bank_type == $bank->id ? 'selected':''}}>{{$bank->name}}</option>
                            @endforeach
                        </select>
                ...
                <h5 class="mt-3 mr-3"><span class="font-weight-bold text-primary">المبلغ في البنك</span> : <span class="total-price">{{number_format($total)}} جنيه</span></h5>
            </div>
        </div>
    </div>
</div>
