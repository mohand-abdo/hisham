    ...
    <div class="col-12">
        <div class="card shadow mb-4 ">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary" style="float:right">حساب الايرادات</h6>
            </div>
            <div class="card-body">
                @include('includes.messages')
                <div class="col-12">
                    <form class="d-flex justify-content-between mb-3" method="post" action="{{route('report.wared.search')}}"  >
                        @csrf
                        <div class="col-5">
                            ...
                            <tbody>
                                @foreach ($wareds as $index => $wared)
                                    <tr>
                                        ...
                                        <td>{{ $wared->bank_id ? $wared->bank->name : '-'}}</td>
                                        <td>{{ $wared->note}}</td>
                ...
                <h5 class="mt-3 mr-3"><span class="font-weight-bold text-primary">الايرادات</span> : <span class="total-price">{{number_format($total)}} جنيه</span></h5>
            </div>
        </div>
    </div>
</div>
