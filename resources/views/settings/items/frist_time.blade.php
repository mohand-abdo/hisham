...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary" style="float:right">بضاعة اول المدة</h6>
        </div>
        <div class="card-body">
            @include('includes.messages')   
            <div class="tab">
                <form action="{{route('items.qty.store')}}" method="post">
                    @csrf
                    ...
                    @foreach($stocks as $index =>$stock )
                        <div id="tab{{$index + 1}}-content" class="tab-content"> 
                            @component('components.tables.frist_time',['items'=>$items , 'stock' => $stock])@endcomponent
                        </div>
                    @endforeach
